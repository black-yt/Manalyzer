...
仓库当前没有 `requirements.txt`、`pyproject.toml` 或 `environment.yml`。因此依赖需要根据源码中的 import 反推安装。建议使用独立虚拟环境，避免污染系统 Python。
...
还需要安装或提供 `structai`。该库在当前仓库中没有定义，但主流程大量使用它：
...
如果当前环境没有 `structai`，主流程无法真正运行。烟测中的部分测试会使用 fake `structai`，因此烟测可以在缺少该依赖时覆盖一些静态和局部行为，但不能替代完整运行。
...
1. 已安装项目所需依赖，尤其是 `structai`、`arxiv`、`openai`、`pandas`、`scikit-learn`、`matplotlib`、`Pillow`、`python-Levenshtein`、`markdown` 和 `beautifulsoup4`。
2. 已配置 LLM 服务环境变量：
...
### 7.5 DataExtratorWithChecker：数据抽取与自检
...
可视化 WebUI 运行与第 6 节的命令行完整运行使用同一条 Manalyzer Pipeline。论文收集、PDF 解析、论文评审、图表转写、数据抽取、数据合并、分析可视化和报告生成的阶段顺序保持一致。
...
原因是 `structai` 不是当前仓库的一部分，也没有依赖清单自动安装。需要安装兼容版本，或确认运行环境中已经提供该库。
